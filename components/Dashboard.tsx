...
const Dashboard: React.FC<DashboardProps> = ({ students, classes, payments, instructors, costs, nuptialDances, setView, addPayment }) => {
    ...
    // 5. New Metrics: ROI & Collection Rate
    // ROI: (Beneficio / Costes) * 100.
    const roiPercentage = totalCosts > 0 ? ((totalProfit / totalCosts) * 100) : 0;

    // Collection Rate: (Cobrado Año Actual / (Cobrado Año Actual + Pendiente Año Actual)) * 100
    // Usamos el contexto del año actual para ser consistentes con la deuda calculada.
    ...
    return (
        <div className="p-4 sm:p-8 space-y-8 pb-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {/* NEW CARDS */}
                <StatCard title="ROI (Global)" value={`${roiPercentage.toFixed(1)}%`} subtext="Retorno inversión" color="indigo">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                </StatCard>
                <StatCard title="Tasa de Cobro" value={`${collectionRate.toFixed(1)}%`} subtext="Año actual" color="teal">
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
